...
namespace VehicleBehaviour.Trails
{
	...
	public class TrailEmitter : MonoBehaviour {
		...
		void Update () 
		{
			...
			if (!trailing && wheel.isGrounded && (Mathf.Abs(hit.sidewaysSlip) > 0.7f || Mathf.Abs(hit.forwardSlip) > 0.98f || vehicle.drift) && hit.collider != null && hit.collider.CompareTag("Static")) {
				trailing = true;
				NewTrail ();
			} else if (trailing && (!wheel.isGrounded || (Mathf.Abs(hit.sidewaysSlip) < 0.5f && Mathf.Abs(hit.forwardSlip) < 0.98f) && !vehicle.drift) || hit.collider == null || !hit.collider.CompareTag("Static")) {
				trailing = false;
				EndTrail ();
			}
